...
bool settings::set_preference(dpp::snowflake user_id, setting setting, bool option)
{
    ...
    for (const string &query : requests.value())
    {
        char *error_message = nullptr;
        int rc = sqlite3_exec(db, query.c_str(), NULL, 0, &error_message);
        if (rc != SQLITE_OK)
        {
            cerr << "Error on: " << query << endl;
        ...
        else
        {
            ...
            this->cached_user_settings.at(user_id).last_accessed = std::chrono::system_clock::now();
...
void settings::get_user_settings(dpp::snowflake user_id)
{
    ...
    string query = format("SELECT * FROM settings WHERE user_id = {}", id);

    char *err_msg = nullptr;
    int rc = sqlite3_exec(this->db, query.c_str(), [](void *data, int argc, char **argv, char **azColName)
                          {
    ...
    user_settings.last_accessed = std::chrono::system_clock::now();
    ...
    return 0; }, &this->cached_user_settings, &err_msg);

    // Check for database errors
    if (rc != SQLITE_OK) {
        cerr << "SQL Error in get_user_settings: " << err_msg << endl;
    ...
    {
        uint64_t id = static_cast<uint64_t>(user_id);
        std::string query = std::format("INSERT OR IGNORE INTO settings (user_id) VALUES ({});", id);
...
void settings::evict_old_cache()
{
    auto now = std::chrono::system_clock::now();
    for (auto it = cached_user_settings.begin(); it != cached_user_settings.end();) {
        auto age = std::chrono::duration_cast<std::chrono::hours>(now - it->second.last_accessed).count();
        if (age > 24) {
            it = cached_user_settings.erase(it);
        } else {
            ++it;
        }
    }
